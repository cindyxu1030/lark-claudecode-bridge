...
- Add one or both bots to a group. No mention means every bot responds; `@Codex` only routes to Codex; `@Claude` / `@Claude Code` only routes to Claude
...
# Keep LARKSUITE_CLI_BRAND=lark for Lark international, or set it to feishu.
...
3. **Enable permissions** — Permissions & Scopes (权限管理):
   ...
   | `im:message.reactions:write_only` | Add bot emoji reactions |
...
| `LARKSUITE_CLI_BRAND` | no | `lark` | `lark` for Lark international, `feishu` for Feishu China |
| `LARK_BRIDGE_ENV_FILE` | no | `.env` | Alternate env file, useful when one clone runs separate Claude/Codex apps |
...
To reproduce the two-bot group setup, create **two Lark/Feishu apps**: one for Claude and one for Codex. Each app needs the bot feature, the same message events, and the same IM scopes. Add both bots to the same group chat.

Use either two runtime folders, each with its own `.env`, or one clone with two env files:
...
Set different app credentials in each file. In `.env.codex`, also set:

```bash
DEFAULT_MODEL=gpt-5.4
SESSIONS_DIR=~/.lark-codex
CODEX_HOME=~/.codex
```
...
```bash
LARK_BRIDGE_ENV_FILE=.env.claude python main.py
LARK_BRIDGE_ENV_FILE=.env.codex python main_codex.py
```

Routing rules in group chats:

```text
No bot mention: both bots respond
@Codex: only Codex responds
@Claude or @Claude Code: only Claude responds
```

`/discuss <topic>` is coordinated by Codex by default so the group gets one summary instead of two duplicate summaries.
...
- 一个群里可以放一个或多个机器人。不 @ 时所有机器人都会回复；只 @Codex 时只有 Codex 回复；只 @Claude / @Claude Code 时只有 Claude 回复
...
# Lark 国际版保持 LARKSUITE_CLI_BRAND=lark；飞书国内版改成 feishu。
...
3. **开启权限**（权限管理页面）：
   ...
   | `im:message.reactions:write_only` | 机器人添加消息表情回复 |
...
4. **开启长连接模式**
   ...
   - 如需文档评论回复，再添加：`drive.notice.comment_add_v1`
...
| `LARKSUITE_CLI_BRAND` | 否 | `lark` | Lark 国际版用 `lark`，飞书国内版用 `feishu` |
| `LARK_BRIDGE_ENV_FILE` | 否 | `.env` | 指定另一个 env 文件；一个 clone 同时跑 Claude/Codex 时使用 |
...
如果要复现一个群里同时和 Claude、Codex 对话的模式，请创建**两个 Lark/飞书应用**：一个给 Claude，一个给 Codex。两个应用都要开启机器人能力、相同消息事件和相同 IM scopes，然后把两个机器人都拉进同一个群。

可以用两个运行目录分别放 `.env`，也可以一个 clone 里放两个 env 文件：
...
不 @ 机器人：两个机器人都回复
@Codex：只有 Codex 回复
@Claude 或 @Claude Code：只有 Claude 回复
```

`/discuss 主题` 默认由 Codex 协调，因此群里只会收到一份讨论总结。
...
| `/discuss 主题` | 让 Claude 和 Codex 讨论，返回总结，并保存完整记录到 `DISCUSSIONS.md` |
